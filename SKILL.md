...
- 🔴 **「資料救不回來」是最貴的結論，宣告前先證明你打開過它**：它把成本推給人
  （回源重抓、甚至跑一趟公部門），所以舉證責任最高、不是最低。實測 2026-08-07：
  「31 卷原卷不存在」三次判斷**全部**是工具缺陷（入口 regex 只認開頭、括號寬度、
  科目欄是空的），紙一卷都沒少。閘門：掃描器必須同報「處理 N／跳過 M＋原因」，
  unrecoverable 清單必須逐項附「試了哪個路徑、失敗原因」。→ `parsing.md` §11.5
...
| `references/parsing-lessons.md` | `references/parsing.md` 的教訓日誌（按事故日期；節號沿用正本） |
| `references/figures.md` | 裁圖（整塊 render）、定位、**圖→題對應隨考試家族變**、**KaTeX 分流與假圖訊號**、band 邊界、base64 安全架構、保真 |
| `references/figures-lessons.md` | `references/figures.md` 的教訓日誌（按事故日期；節號沿用正本） |
...
| `references/dirty-data-robustness-lessons.md` | `references/dirty-data-robustness.md` 的教訓日誌（按事故日期；節號沿用正本） |
