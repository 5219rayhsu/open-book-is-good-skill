...
## 🔴 載入後動工前：先做一次在地化（給 agent 看的）

**本 skill 只寫方法與判準，不寫「你該用哪個模型、哪個 effort、哪個後端、門檻設幾 %」。**
那些會隨供應商、方案、額度制度而變，抄過來的當天就開始腐爛。所以凡是 `references/` 裡
出現具體配置的地方（例如 `batching-and-measurement.md` §23 的後端閘與額度閘），
**請依你自己的 agent 與執行環境重新配置一次**，再開始跑：

| 要在地化的東西 | 怎麼決定 |
| --- | --- |
| 後端（哪家 CLI／API）與它的**預設值** | 預設值一律指向**使用者當前指定的那條路線**，不是你最順手的那條 |
| 模型與 effort 的具體組合 | 逐站指名，不給預設值。改動一組配對是一次量測，不是改一行預設 |
| 額度／配額怎麼讀 | 先找你的後端**已經寫下的副產物**（log、rollout、usage 檔），再考慮發請求 |
| 停跑門檻（開新工作／收尾地板） | 依你的方案與重置週期自訂；用「剩餘」表述，不要用「已用」 |
| `scripts/` 的路徑、檔名、科目假設 | 依你的資料源改寫，不能原封不動照跑 |

**判準與閘門的形狀請照抄，數字與名字請自己填。** 原則沒有保鮮期，模型名有。
...
| [`references/explanations-redteam.md`](references/explanations-redteam.md) | clean-room 詳解、模型路由、pipeline token 工程（大批量省 token 槓桿）、反駁式紅隊、Wilson 停止規則、節制門、lint、作文政策 |
| [`references/build-deploy.md`](references/build-deploy.md) | build 單檔／網站、PWA／Service Worker、體積永續 |
| [`references/dirty-data-robustness.md`](references/dirty-data-robustness.md) | 髒資料韌性：政府檔名／版面漂移的寬鬆匹配、缺檔必報錯、寧可崩潰不可靜默漏抓 |
| [`references/shared-question-banks.md`](references/shared-question-banks.md) | 跨考試共用題庫：開新考試前先做科目重疊矩陣，重疊科目建一次、兩考試共用 |
| [`references/frontend-engine-rules.md`](references/frontend-engine-rules.md) | 前端引擎規則：題組情境引入、出題演算法、標記與儲存的跨作答表面一致規則 |
| [`examples/walkthrough.md`](examples/walkthrough.md) | 端到端範例：學測社會 111（最難啃的一科） |
| [`examples/pipeline-templates.md`](examples/pipeline-templates.md) | 管線範本示範：操作卡、金樣詳解、錨定素材包各一則 |
| [`docs/adr/`](docs/adr/) | 方法論決策紀錄（ADR）：各項管線取捨「為什麼這樣做」；改方法論前先讀 |
| [`CONTRIBUTING.md`](CONTRIBUTING.md) | 如何參與、三層授權、標點規範、改方法論前先讀 ADR |
...
歡迎回報問題、補案例、修正規則。開始前請讀 [`CONTRIBUTING.md`](CONTRIBUTING.md)（如何參與、三層授權、
標點規範）；**要改方法論規則，先讀 [`docs/adr/`](docs/adr/) 對應的決策紀錄**——很多規則是實測踩坑後
刻意保留的，ADR 記了它們為何長這樣。
...
- **AI 生成內容或衍生物**（例如 `examples/sample-output/` 內由 AI 生成的詳解切片，以及依本手冊生成的選擇題／申論題解析）：屬衍生物，採 **[CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/)**（公眾領域貢獻）——複製、修改、散布、商業利用皆可，不需許可，也不需標示來源。這類內容一律標「AI 整理、須查證」與把握度，不是官方標準答案。
