...
沿用本檔的「原生物件層優先」——分數線就是一條細長水平向量。四類靠幾何分開，
**門檻要拿真實座標量，不要憑感覺**（第一版把容差寫成 3.0，於是每一條分數線都被
判成上槓；實測兩群是 0.35–0.47 與 1.54–1.58，差三倍且不重疊）：

| 類別 | 上方有字 | 下方間距 | 左緣有向下筆畫 |
| --- | --- | --- | --- |
| 分數 `\dfrac{}{}` | 有 | ~1.55pt | — |
| 根號 `\sqrt{}` | 無 | ~0.4pt | **有** |
| 線段記號 `\overline{}` | 無 | ~0.4pt | 無 |
| 表格線／底線 | — | 沒有被它框住的字 | — |
