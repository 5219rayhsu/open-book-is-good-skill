# 圖式科：裁圖內嵌 + base64 安全架構


## 目錄

> `head -100` 看不到全貌，故列出所有節名——**用節名搜尋，不要從頭讀**。

- 一、為何不靠 `page.get_images()`
- 二、怎麼定位題目在頁面的位置
- 二之一、圖→題對應隨考試家族變，別套同一管線
- 二之二、定位錨點、假圖訊號與 KaTeX 分流
- 三、band 邊界怎麼決定（不切半張圖）
- 四、★ base64 安全架構（最重要）
- 五、保真，不做失真壓縮
- 六、冪等與重用
- 七、堆疊的數學結構：分數、根號、線段記號（2026-08-18 實測，分科測驗數學）
- 八、紅色浮水印濾除：色彩判準要用通道飽和度，不是色相或「非中性色」
- 九、掃描卷沒有物件層可查，改用墨跡列高度剖面找圖的上下界

---

> **裁圖/圖表抽取的「主」參考＝ `exam-pdf-asset-extractor` skill**（圖表是它的專責：像素 autocrop＋裁切偵測、image/text 雙錨點開關、image-table text-pair 錨點、跨頁、Completeness Re-check 掃全題型）。新裁圖工作先讀那支。本檔只留 **open-book 專屬建置面**：離線單檔（學測／會考）的 base64 內嵌架構；**國考是線上站 → 圖存 PNG 不做 base64**（見主 skill）。
>
> 適用範圍：社會圖表題、英文圖選項題、國文／社會題組共用材料圖，以及未來的數學／自然圖式科。
> 對應腳本：`scripts/extract_figures.py`（學測）、`scripts/extract_figures_cap.py`（會考）、`scripts/build_app.py`（建置內嵌）。
> 授權：MIT。

圖式題若缺圖等於廢題 —— 練習要面對真實，所以本專案的取向是**裁切原卷版面區塊 render 成 PNG 內嵌**，忠實重現原卷，不簡化、不重排。本文件說明兩件事：

1. **怎麼把圖從 PDF 裁出來**（為何不靠 `get_images()`、怎麼定位題目、怎麼決定 band 邊界）。
2. **★ base64 安全架構**（最重要）：圖檔、題庫、建置三者如何分工，讓 base64 全程不進對話／終端。

---

## 一、為何不靠 `page.get_images()`

直覺做法是用 PyMuPDF 的 `page.get_images()` 把頁面內嵌的點陣圖一張張抽出來。但考卷的圖**多半抓不到**：

- 地圖、統計圖表、示意圖、路線圖，原卷裡常是**矢量繪製**（向量路徑 + 文字輪廓），根本沒有一張可被 `get_images()` 列舉的內嵌點陣圖。
- 就算抽得到內嵌點陣，也會漏掉疊在上面的座標軸文字、圖例、比例尺這些**向量層**，拼不回完整的圖。

**改用整塊 render**：用 PyMuPDF 把「題目所在的版面區塊（band）」**整塊點陣化** —— raster 與 vector 一起進同一張 PNG，所見即所得。render 參數：

```python
ZOOM = 2.2
MATRIX = fitz.Matrix(ZOOM, ZOOM)
# render：page.get_pixmap(matrix=MATRIX, clip=fitz.Rect(xL, y0, xR, y1))
```

`zoom 2.2` 是清晰度與體積的折衷：夠看清地圖細節，又不至於讓單張 PNG 過肥。clip 框出 band 的矩形範圍，`get_pixmap` 把那塊版面（含所有圖層）一次點陣化成乾淨 PNG。

---

## 二、怎麼定位題目在頁面的位置

要裁對 band，先得知道「這一題從頁面哪個 y 座標開始」。兩支腳本用了不同但同源的策略。

### 學測：用 stem／題組標記的文字前綴定位（`extract_figures.py`）

逐頁建一個「**正規化字元索引**」：`build_char_index(page)` 走訪 `page.get_text("dict")` 的每個 text span，把每個非空白字元對應到它所屬 span 的 bbox，得到 `(normtext, metas)`。接著：

- **獨立題**：用題目 `stem` 的正規化前綴在 `normtext` 內 `find` 出 y 座標（`locate_text`）。關鍵是**長前綴優先 + 要求唯一**：先試 30→10 字且該前綴在頁內唯一（避免短字串或題號數字撞到別題）；對不上才放寬到 6 字、不要求唯一。
- **題組共用圖／材料**：改用「**N-M為題組**」這個題組標記當主錨點（`find_group_mark`）。它文字短、固定、落在 passage 區上緣，比拿 passage 全文比對可靠得多 —— passage 經 markitdown 重排後，字序與 PDF span 已不一致，且常夾雜圖說／數字而比對失敗。
- **fallback**：bank 裡若某題 stem 開頭被解析器截斷，`locate_text_fragments` 把 stem 切成多個 12 字片段，任一命中即可，並用 `snap_to_question_number` 把上界吸附回該題的「`{no}.`」題號行，讓 band 含到題號。

### 會考：用題號行定位（`extract_figures_cap.py`）

會考版面與學測不同：題號行「`{no}.`」於左邊界（`x0 < QNUM_X_MAX = 92`）非常穩定，所以**改用題號行當主錨點**。

- `collect_question_anchors(page)` 掃每行，行首符合 `QNUM_RE = ^\s*(\d{1,3})[.．]` 且左邊界夠左者，收成 `(no, y0)` 錨點清單。
- 為什麼不用 stem：會考「這張圖最可能…」型題，**圖夾在題號行與 stem 之間**，若用 stem 當上界會把圖切掉。用題號行當上界才能含進圖。
- 題組材料用「**…回答第 {首} 至 {末} 題：**」這類子標題定位（`find_group_head`，容忍 `~ ～ - － 至 到` 各種分隔符），找不到子標題時退而用第一子題題號行往上抓一段保守 padding。

> **題組圖 vs 獨立題圖的擺位原則**：題組共用圖屬於整組的材料，放在**文章（passage）區**；獨立題的圖屬於該題，放在**題幹之後**。腳本據此選擇上界錨點（題組→題組標記／子標題；獨立→stem 或題號行），確保裁出的 band 對應到正確的視覺脈絡。

---

## 二之一、圖→題對應隨考試家族變，別套同一管線

- **題組共用圖 → 寫進全組子題**：一張題組共用圖**只裁一次**（放在實際出現的子題位置），
  但 `figure` 欄要 **propagate 到該 `group_id` 的每一個子題**。偵測器只給
  「圖 → 所在題號」這一筆，group 補齊是 **wire 進 bank 這步用 `group_id` 做**
  （偵測器不知道分組）。不補的話，可單獨出的子題（`standalone=true`）在模擬／單題／
  錯題／弱點被單獨抽出時就缺圖。
  🔴 **這條規則的前提是「全組真的共用同一張」，broadcast 前要先驗證這個前提**
  （2026-08-25 實戰，`figdet_wire.py` 的 group-figure propagate bug）：學測 gsat
  實測 6 個 group／8 個 qid，題組內其實有**不同的圖**（同組不同子題各自指涉
  不同的 `圖N`），但抽取管線只裁到其中一張，就把那一張的檔名整批 propagate 給
  全組——其餘子題掛著一張跟自己內容完全不相干的圖，而 `needs_figure`／
  `parse:ok` 兩道既有閘都看不出異常（欄位齊全、檔案存在，只是**指錯圖**）。
  修法：propagate 前**比對 anchor 與該 member 自己引用的圖／表標籤**
  （`圖N`／`表N`）是否一致；引用的標籤不同 ⇒ 降級為 `review`，不要無腦廣播
  anchor 自己的圖。判準：**「同一個 group_id」不等於「同一張圖」**，
  題組共用材料（passage、法規全文）可以整組共用，但**題組內每個子題各自的圖
  仍要各自核對**，把兩者混為一談的假設要在偵測與 wire 兩端都能被戳破。
  單一共用素材、具名衍生檔與單題否證慣例見 skill `exam-pdf-asset-extractor` 的「題組共用圖：同一完整素材只裁一份」。
- **數學 vs 非數學是兩條管線**：數學圖在題幹下方（對最近上方題號）；非數學圖常是
  **題號上方的共用材料**或相鄰單題共用而無 group → 用
  **caption-anchored**（讀圖的 `圖N` caption，對應引用該標籤的題）優先；
  無標籤（如會考多為指示詞）才退 **position**（圖在頁頂→給下方首題）。
- **`needs_figure` 非數學雙向都會錯**：漏抓（寬圖被濾、頁頂圖丟）要回收、
  假旗標（純文字題／`\dfrac` 公式題）要降旗。
- ⇒ **每個（考試家族 × 科）先抽 1–3 卷判**：`needs_figure` 是否整批盲標、
  有無 `圖N` 標籤、圖在題上還是題下，再選方法。
  完整見 skill `exam-pdf-asset-extractor` 的「Mapping figures to questions」
  ＋「What varies per exam family」。

---

## 二之二、定位錨點、假圖訊號與 KaTeX 分流

- **轉寫過的題幹定位會失敗（數學最慘）**：LaTeX／清洗過的 stem 對不上 PDF 文字層，
  會出現「stem 無法定位」或抓到純文字假圖。
  🔴 **band 偏矮＝抓錯區塊的警訊**，與裁切到圖同一類，commit 前一起複查。
  定位錨點改用**題號＋符號前的純散文前綴**（或 bank 的 `locate_anchor`，
  見 `data-sources.md` §5），別比對整段 LaTeX stem；圖不一定在題幹與選項之間
  （數學常在 stem 上方或題組共用區），真圖改用 image／drawing-rect 錨點。
- **裁數學圖前先看站上的渲染能力**：若部署端已用 KaTeX／MathJax 渲染 LaTeX，
  許多數學 `needs_figure=true` 是舊離線單檔版「把公式截成圖」的遺留——
  **重審後降旗、交給渲染器畫**，只有真幾何圖（多面體、座標圖、電路／結構圖）才裁。
  對每個數學 `needs_figure` 硬裁只會產生純文字假圖。
- **假圖訊號（答題說明／劃記 boilerplate）**：升學試卷 Q1 前常有獨立的
  「選填題答案卡劃記說明」頁（網格＋劃記範例），對任何墨偵測都像圖、是強假陽性
  （每份學測數學 p0 都有）；國考考選部試題 PDF 則乾淨（直接從第 1 題起、答案卡另份）。
  **第一道防線＝跳過無題號頁＋圖必對應某題**，即可濾掉。詳見
  skill `exam-pdf-asset-extractor` 的「Known False-Figure Signals」
  ＋「Erase-Text Figure Detection」。

---

## 三、band 邊界怎麼決定（不切半張圖）

定位到上界後，下界與圖底保護是裁圖品質的關鍵。

**上界**：題目／題組標記起點往上留少量 padding（`TOP_PAD`），但不高於頁眉下緣（`HEADER_Y`）。會考另有 `band_image_top`：右側並排圖的頂端常凸出於題號行上方，往上延伸把圖含進來，但絕不越過前一題。

**下界**：取「**同頁下一個題目／題號行的起點**」當硬上限（確保不吃進下一題的圖）；若是該頁最後一題，則到頁尾內文底部（`page_text_bottom`，已排除頁碼）。

**圖底保護（最關鍵）**：`band_image_bottom` 掃 band 內的圖片（`get_images`）與向量圖（`get_drawings`），對「圖頂落在 band 內」的圖取其底部 y 的 max，把下界往下延伸，**確保圖不被切半張**；但延伸絕不超過硬上限（下一題起點），避免吃進下一題。整頁框線（高度超過頁面 85% 的大矩形）排除，不誤判外框為圖。

🔴 **冒字防呆的夾線要用結構座標，不能用「猜哪些文字屬於本題」**（2026-08-25 實戰，
藥師科 `extract_pharmacist_figures.py` 的 `nearby_text_clip_limits()`）：這支函式
原本只靠 `source_lines_in_span()` 抓「屬於本題」的殘餘文字行當夾線，圖的是避免
裁圖冒出不相關的字。但下一題錨點行的中點，結構上必然落在下一題那一側，**永遠
不會被「找出屬於本題的文字」這條路徑看到**——於是固定 padding 一路延伸下去，
把下一題題號那一行的文字整行渲成清晰可讀的字，混進本題的裁圖裡。3 題實測中招
（跨頁或緊貼下一題的多圖版裁圖）。修法：改用**上一題／下一題錨點自己的
`segment.y0`／`segment.y1`** 當硬夾線，不必先判斷「這段文字是不是本題的」、
不必分辨版面或標記格式——**邊界問題優先用「相鄰錨點的座標差」解，不要用「內容
辨識＋文字歸屬猜測」解**：前者是幾何事實，量得到就不會錯；後者永遠有辨識不到、
辨識錯誤的殘餘案例，而且案例只會隨版面變體增加，不會收斂。

會考題組另有更細的判斷：用 `figure_segment_count`（向量線段 + raster 圖加權計數）判斷「圖在 passage 還是在某子題」，再決定要不要把下界延伸到含圖子題之後 —— 純文字 passage 的題組，真正的圖常落在某個 `needs_figure` 子題的題幹旁或選項區。

### 寬度：一律整頁寬單欄

社會／英文／國綜／會考的題幹經探勘皆為單欄佈局。**刻意不依文字 x0 切欄** —— 題組材料常是「文字左、圖右」的圖文並排，依文字切欄會把右側的圖切掉（實測 `111_社會_g59_60` 的圖 11 在右半被誤切）。故 band 一律取整頁寬（扣左右白邊 `SIDE_MARGIN_L/R`）。`detect_columns` 保留為單一寬度決策點，日後真遇雙欄題卷可在此擴充。

### 檔名規則（跨科一致、含年份與科目）

> **本節是命名規則的正本**（`data-sources.md` 只留指標）。2026-08-23 用 `bank.json` 的
> `figure` 欄逐筆量過：舊版寫的 `S114_q24.png`／`C111_國文_q1.png` 這種「簡碼＋年份」
> 命名，**在實際題庫裡一張都沒有**（gsat 219 個不同檔名、cap 402 個，符合舊規則者 0）。
> 檔名不是規範出來的，是**產出它的那支腳本決定的**，所以實際上有三個家族並存。

**量測結果（2026-08-23，數字＝不同檔名數／該庫全部不同檔名）：**

| 家族 | 樣式 | 實例 | gsat | 會考 cap |
| --- | --- | --- | --- | --- |
| **A 原始渲染名（主力）** | `{PDF stem}_p{頁}_q{題}_{序}.png` | `111_社會_試題_p6_q24_0.png`／`111_會考國文_試題_p1_q1_0.png` | 166／219 | 311／402 |
| **B 管線批次名** | `{exam}_{科}_{年}_p{頁}_{圖\|表}{編號}_{序}.png` | `gsat_社會_111_p4_圖3_0.png`／`cap_社會_111_p1_圖二_2.png` | 41／219 | 46／402 |
| **B′ 定位批次名** | `{exam}_{科}_{年}_pos_p{頁}_q{題}_{序}.png` | `gsat_社會_112_pos_p9_q36_0.png` | 5／219 | 22／402 |
| **C 單點補抽／重裁** | 前三者以外的手工名 | `113_國綜_15_sidebox.png`、`115_英文_38_figopt.png`、`112_會考自然_試題_g47_48_passage.png`、`cap_社會_111_圖十四_re.png` | 7／219 | 23／402 |

- **A 的 `{PDF stem}`＝原卷 PDF 檔名去副檔名**：學測 `{年}_{科}_試題`、會考 `{年}_會考{科}_試題`
  （科名含「會考」二字，不是分開的欄位）。產生處：`extract_figures_smart.py:222`
  與 `_build_features/figdet_extract.py:57`，兩者同一行式 `f"{tag}_p{pno}_q{qno}_{k}.png"`，
  `tag` 取 `os.path.basename(pdf)` 的 stem。
- **B／B′ 出自 skill `exam-pdf-asset-extractor` 的 `figdet_v3.py`**：命名在
  `~/.claude/skills/exam-pdf-asset-extractor/scripts/figdet_v3.py:325`
  （`fn = f"{tag}_p{a['page']}_q{a.get('question')}_{a['kind']}_{i}.png"`），`tag` 在同檔
  `:355` 組成 `f"{a.exam}_{a.subject}_{a.year}"`。落到 bank 的檔名是該 manifest 的
  `output` 欄被 wire 回 `figure`；`圖3`／`表5`／`圖二` 這段來自圖說標籤、`pos` 來自定位批次的
  `tag` 後綴（**讀碼推得，未逐張回溯批次紀錄**）。
- 🔴 **頁碼基準兩家不同（讀碼推得）**：A 家族的 `p{頁}` 來自 `range(len(doc))`，**0-indexed**；
  B 家族的 `a["page"]` 在 `render_assets` 裡以 `doc[a["page"] - 1]` 取頁，**1-indexed**。
  拿檔名回推原卷頁碼時先看是哪一族，否則差一頁。
- **C 是修復站與補抽留下的**：`_figopt`（圖片選項重裁）、`_sidebox`（補充框）、
  `_passage`／`_all`（題組整塊）、`_poster`、`_re`（重裁）、`_fig`
  （`_build_features/recover_missing_figures.py:482` 的 `f"{qid}_fig.png"`）。
  它們是既成事實，**不要為了「統一」去改名**——`figure` 欄是唯一指標，改名等於斷鏈。
- **題組共用圖**：group 內所有 `needs_figure` 子題的 `figure` 欄都指向同一張。少數以
  `g{首}_{末}` 命名的（實測只有會考自然 4 張，如 `112_會考自然_試題_g47_48_passage.png`），
  區間取該 group 全部成員題號的 **min~max**（忠實：共用同一張圖）。

#### 🔴 檔名裡的「圖 N」不是證據，裁切位置才是（2026-07-31 實戰）

`gsat_自然_111_p2_圖3_0.png` 裝的其實是**圖 2**（讀卡機／磁場／晶片示意圖）。
真正的圖 3（地球氣候系統模型）在同一頁下半部，圖說在 y=743.9。
`111_自然_7`／`8` 兩題於是對著一張完全不相干的圖作答。

**這個錯誤所有既有的閘門都看不到**：檔名格式對、檔案存在、圖沒破、
留白均勻、題幹確實提到「圖 3」——每一條規則都通過。

因為它們驗的都是**檔案本身**，沒有一條驗「這張圖**是不是**它宣稱的那張」。

**補一道閘門：裁切區必須含住該圖的圖說。**

```python
caps = [fitz.Rect(w[:4]) for w in pg.get_text("words") if w[4].strip() == label]  # label = "圖3"
assert caps, f"F1：頁 {pno} 找不到圖說「{label}」"
cap = min(caps, key=lambda r: abs(r.x0 - box[0]))
assert rect.contains(cap), f"F1：裁切區沒含住「{label}」的圖說"
for o in other_caption_rects:            # 圖1/圖2/圖4…
    assert not rect.intersects(o), "F2：裁切區與別的圖說重疊，可能一次吃了兩張圖"
```

**圖說是這張圖唯一的自我宣告**，把它含進裁切區，等於讓資產自己帶著身分證。
副作用還很好：使用者在網頁上看得到「圖 3」，跟題幹的「依據圖 3」對得上。

> 一般化：**命名是人給的，位置是版面給的。** 凡是「檔名／欄位宣稱它是 X」的地方，
> 都要問一句「有沒有一道檢查真的驗過它是 X」。沒有的話，那個名字只是註解。
> 這一類錯誤只有**把圖畫出來用眼睛看**才會發現——contact sheet 不是可選步驟。

---

## 四、★ base64 安全架構（最重要）

base64 字串塞進對話／終端／工具輸出會觸發 Anthropic AUP 內容過濾器、kill 掉 session —— 這是專案鐵則。整套設計就是要讓 **base64 全程不進那些通道**。資料流分三段、職責清楚：

```
┌──────────────┐   裁圖     ┌──────────────────┐  只寫檔名  ┌──────────────┐
│  原卷 PDF     │ ────────▶ │ data/figures/*.png │ ────────▶ │  bank.json    │
│ data/raw/    │  (render)  │  獨立二進位 PNG     │  (純文字)  │ figure 欄存檔名 │
└──────────────┘            └──────────────────┘            └──────┬───────┘
                                      │                            │ build 時
                                      │  build 讀檔                 │ 讀檔名
                                      ▼                            ▼
                            ┌─────────────────────────────────────────────┐
                            │  build_app.py：figures_block()                │
                            │  PNG bytes → base64 data URI                  │
                            │  → window.__FIGS__ = {檔名: dataURI}          │
                            │  ★ base64 只在此函式內部 + 輸出 HTML 流動       │
                            └─────────────────────────────────────────────┘
```

### 1) 圖存獨立 PNG（`data/figures/檔名.png`）

裁圖腳本一律把圖存成**獨立的二進位 PNG 檔**，絕不把圖內容（bytes／base64／data URI）印出。腳本的 stdout **只**有：張數、位元組大小、檔名、座標數字、qid 清單、計數。驗圖要靠**開啟 PNG 檔**看，不是把內容印到終端。

### 2) bank.json 只存 figure 檔名（純文字、可 lint）

裁完後，`figure` 欄寫回的是**檔名字串**（例 `"figure": "S114_q24.png"`），永不寫 base64。好處：

- bank.json 維持**純文字**，可被全形／半形標點 lint、可 diff、git 能 delta。
- 題庫與圖的繫結是「檔名指向」，一張圖被多個題組子題共用時，多筆 `figure` 欄指同一檔名即可。

🔴 **「git 能 delta」不等於「git 能溯源到單一 qid」**（2026-08-25 實測）：bank.json 是
**單行** minified JSON——任何一次編輯，不管改的是哪個 qid，在 git 眼裡都是「這一整行被
換掉」。想用 `git log -S'某檔名字串'` 或逐行 `git blame` 反查「是哪個 commit 把這個
figure 值寫進哪個 qid 的」，兩者都會失靈：`blame` 只能定位到「整行」（＝全檔）；
`-S` 的 pickaxe 在同一份跑過的實測裡，對三個完全不同的目標字串回傳了一模一樣的
commit 清單（凡是動過 bank.json 的 commit 全部命中，等於沒有篩選力）。
**「純文字可 diff」講的是「換行後前後兩份檔案可以整份比對」，不是「可以查到誰在何時
改了哪一筆」**——後者需要另外的機制（例如落地時另存一份 per-qid 的修改前後快照，
不能事後指望 git 幫你查）。debug 群組圖繼承一類的「這個值是哪裡寫進來的」問題時，
先假設 git 這條路走不通，改讀程式碼推導機制，別把時間花在對著同一批 commit
清單反覆重跑 pickaxe。

學測腳本把 `figure` 欄寫回 `data/bank.json` 正本；會考腳本只動 `data/_stage/cap_{科}.json` staging，不碰正本。

### 3) build 時才 base64 內嵌進 `window.__FIGS__`

只有建置這一步才碰 base64。`build_app.py` 的 `figures_block(bank)`：

- 收集 bank 中所有題目引用到的 `figure` 檔名（去重）。
- 逐一讀 `data/figures/{檔名}` 的 bytes，編成 `"data:image/png;base64," + base64.b64encode(raw)` 的 data URI。
- 組成 `window.__FIGS__ = {檔名: dataURI}` 一段 `<script>`，注入輸出 HTML。前端用檔名查這個物件拿到 data URI 顯示圖。

函式的**安全紀律寫在 docstring 與行內**：base64 字串**只在本函式內部與輸出 HTML 字串裡流動，絕不 print**。`figures_block` 只 `print` 三件事：內嵌張數／總張數、原始合計大小（MB）、缺檔清單（`figure` 欄指到但 `data/figures/` 沒有的檔名）。

> 同樣的 `</` → `<\/` 轉義由 `js_safe_json` 處理，避免資料裡萬一出現 `</script>` 提早關閉標籤。data URI 經此函式序列化後一起注入。

### 安全紀律小結（agent／貢獻者務必遵守）

- **絕不**把任何 base64 字串放進對話、終端輸出、工具參數、Read 結果。
- 腳本只印**計數與大小**；要看圖就**開 PNG 檔**。
- bank.json／staging JSON 只存**檔名**；base64 只活在 `build_app.py` 內部與最終 HTML。
- 不 `cat` 圖檔、不把圖 bytes 餵進 prompt。

---

## 五、保真，不做失真壓縮

原卷圖表**優先保真**：地圖／圖表一旦失真就影響判讀，所以**不用 pngquant、不降 DPI**等失真壓縮。可做的只有**無損**最佳化（例如 oxipng 重新編碼，像素不變、省約 27%），不算失真。

代價是單檔體積變大（含圖約 19–31 MB／場），且圖式重的科目（數學／自然）還沒加。**「保真 vs 體積」目前無理想解，列為公開待解問題**，向社群徵求做法（向量化、一科一檔再往下切、PWA 隨選載圖等），詳見 `docs/OPEN-QUESTIONS.md`。已知失敗實驗：整頁向量化 SVG 對密集圖表反而**比 PNG 大很多**（密向量臃腫），僅單純幾何線稿可能划算。

---

## 六、冪等與重用

- **學測**（`extract_figures.py`）：每次重建整個 `data/figures/`（清空 `.png` 再產），`figure` 欄以重算後的對應覆蓋寫入。
- **會考**（`extract_figures_cap.py`）：只刪除／重建本腳本負責科目的 `C{年}_{科}_*` PNG，**不動學測的 `S/G/E` 圖**。
- 兩支共用同一套 band／圖底保護機制；`extract_figures.py` 的定位與 render 邏輯**可重用於會考**，會考版即是沿用並按版面差異改寫而成。

### 🔴 派工前先把工具鏈裝滿——沙盒沒有網路，缺一個套件就等於整條支線停擺

裁圖代理跑在**無對外網路**的沙盒裡，它裝不了任何東西。所以「缺什麼套件」不會在
執行期被補上，只會變成一批**看起來像資料缺陷的工單**。

實測 2026-08-09（分科測驗，401 題）：venv 少了 `scipy`，**數學甲／乙 19 題**
全數失敗，理由 `math_erase_text_requires_scipy_unavailable`。代理照操作卡的規矩
**回報工具缺口、沒有交降級品**——這是對的，也是唯一讓人一眼看出真因的做法。
如果它硬湊出 19 張假圖，下游會拿真實資料去修一批根本沒壞的東西。

派工前把 venv 建好、**逐一 import 驗過**：

```bash
uv pip install --python .venv-pdf/bin/python pymupdf numpy pillow scipy
.venv-pdf/bin/python -c "import fitz, numpy, PIL, scipy; print('ok')"
```

| 套件 | 誰要用 | 缺了會怎樣 |
| --- | --- | --- |
| `pymupdf`（`fitz`） | 全部——物件層偵測、bbox、render | 整條線跑不動，會立刻爆 |
| `numpy` | whitespace-trim 的墨跡 bbox、edge-touch 判定 | 裁圖沒有白邊修剪，切邊偵測失效 |
| `pillow` | 存 PNG、contact sheet 拼版 | 產不出驗收用的 contact sheet |
| `scipy` | **只有數學科的 erase-text 路徑**（連通元件標記） | 數學卷靜靜失敗，其他科完全正常 |

`scipy` 特別容易漏，因為**只有數學科用得到**：非數學科全綠，報表看起來很健康，
缺口只出現在一個子集裡。**一個只在部分子集用到的依賴，它的缺席會偽裝成那個子集的
資料特性。** 所以驗證要用 `import`，不要用「跑一科看看有沒有錯」。

**根因不是忘了裝，是換了呼叫慣例。** skill `exam-pdf-asset-extractor` 的指令行本來
把依賴**帶在自己身上**：

```bash
uv run --with pymupdf --with numpy --with scipy --with pillow python …
```

專案為了省下每次解析的時間，改成一個常駐的 `.venv-pdf`。這一步把依賴清單從
**「跟著指令走、不可能漏」**變成**「要有人維護、漏了沒人知道」**——而漏掉的正是
那個只有子集用到的。

> 用共用環境取代 `uv run --with` 這類自帶依賴的呼叫時，**那份 `--with` 清單就是
> 你新環境的驗收清單**，要逐項 import 驗過。省下的是啟動時間，換來的是一份
> 需要維護的隱性契約。

**但清單本身也會漏，所以正解是不要有清單。** 派工閘在發包前，把操作卡引用的每支
腳本丟進指定的 venv 實際 `import` 一次——**不宣告依賴，從程式碼推導依賴**。
這正是 `uv run --with` 原本免費附帶、改常駐 venv 時被丟掉的性質。
兩層才夠：`exec_module` 探針（跟得進 shim 的動態轉發）＋靜態掃描任意縮排的 import
（補得到函式內的延遲 import）。參考實作與它自己的三個盲區見專案的
`_build_features/check_card_deps.py` 與 `IDR-0029`。

🔴 **順帶刪掉操作卡裡「本 venv 已含 X、Y、Z」這類宣告**。實測 2026-08-09：那份手寫
清單漏了 scipy，代理讀了之後**把數學科整條路徑寫死成失敗、連試都沒試**。
**一份宣告環境內容的清單，會讓讀它的人不去試。** 改寫成「依賴齊備由派工閘保證，
真的跑到 `ImportError` 才算缺」。

操作卡裡對應的那條硬規則是：**拿不到指定工具就停下來回報，不要交降級品**——
把工具缺口寫成資料缺陷，成本比明顯的失敗高得多。

### 用法

```bash
# 學測
uv run --with pymupdf python scripts/extract_figures.py
uv run --with pymupdf python scripts/extract_figures.py --dry-run   # 只報告不寫檔

# 會考
uv run --with pymupdf python scripts/extract_figures_cap.py
uv run --with pymupdf python scripts/extract_figures_cap.py --dry-run

# 建置內嵌（base64 只在此步流動，不進終端）
uv run python scripts/build_app.py
```

`--dry-run` 只計算 band 與印報告，不存圖、不寫 JSON，適合先檢查定位與邊界。報告會標出「band 偏矮（疑切圖）」與「band 偏高（疑含到別題）」供人工複核。

### 塗白冒字：判準是「屬不屬於這張圖」，不是「是不是文字」（2026-07-30，一天內踩三次）

裁切區嵌在題幹之間時，標準手法是 render 後把冒進來的文字 bbox 塗白
（CLAUDE.md：subtract text bboxes to kill bleed）。**手法對，判準常寫錯。**

**錯的判準：塗掉裁切區內的所有文字。** 同一天連續反噬三次，每次都是同一個根因——
被塗掉的文字其實是圖的內容：

| 個案 | 塗白毀掉的東西 | 後果 |
|---|---|---|
| 會考海報（111 國文 26–27） | 「洽詢診所服務臺」「收領件時間 08:00~12:00」等 | 海報＝題目資訊，整題答不出來 |
| 圖片選項題（115020 醫三 14） | 選項的**續行**（不以 `A.` 開頭那幾行） | 選項變半句話 |
| 等高線地圖（112 社會 47） | 高程數字 3600／3400／…、圖例、比例尺 | 地圖還在，但「這是哪條路線」無從判斷 |

**對的判準：只塗「以題號開頭的段落」。** 那是唯一確定不屬於圖的東西，而且與
`assert_no_foreign_text`（A2）用的是同一個述詞——一個述詞同時當偵測與修復，不會漂移。
需要更多時，補一條**幾何**規則而不是更多字串規則：宣告這張圖自己的 `content_x/content_y`
範圍，**完全落在範圍外**的文字行一律塗白（那是前段的降部、左欄的題幹）。
幾何規則碰不到圖內文字，因為圖內文字依定義在範圍內。

**規則越窄，誤傷越少。** 每次想把某個字串加進 keep 白名單，都是在提示判準選錯了維度。

#### 兩個實作細節（各自對應一次真實失敗）

1. **逐「行」塗，不逐「段落」塗。** 段落 bbox 是它所有行的聯集，會罩住行與行之間的空隙。
   實測 46 題那一段的 bbox 是 `y381–471 × x65–508`，整片罩住了地圖上方的高程標籤，
   照段落塗會把 `3600/3400/3200` 切掉頭。用 `page.get_text("dict")` 的 `lines[].bbox`。
2. **圖說要用幾何保留，不要用字串前綴保留。** `(圖B)` 在 PDF 裡被拆成 `(圖` 與 `B)`
   兩個 block，比對 `startswith("(圖")` 只會留下半個字。改成「圖底下 `CAPTION_GAP` 點內的
   文字行一律保留」——**位置比字面可靠**。

#### 什麼時候整個不要塗

`optblock`（圖片選項題整塊裁）**不該塗任何字**：整塊就是選項本身，沒有任何文字是冒字，
而 A2 已獨立保證區塊內沒有別題題號。塗白開關要逐版型決定，不是預設行為。

## 七、堆疊的數學結構：分數、根號、線段記號（2026-08-18 實測，分科測驗數學）

裁圖那條路對數學是錯的（KaTeX 會渲染公式，見本檔的 KaTeX 分流）。但**不裁圖不等於
不用讀版面**——分數在 PDF 裡是三層堆疊（分子一行、分數線一條向量、分母一行），
而抽取器照閱讀順序吐字，於是**分子黏在上一個選項的尾巴**：

```
入庫  (A) a 2 > b 2 + c 2 ⏎ c      ← 這個 c 是 (B) 的分子
      (B) > sin 29 ° ⏎ a ⏎ b       ← (B) 其實是 c/a > sin29°
原卷  (A) a² > b² + c²   (B) c/a > sin29°
```

🔴 **它不是缺字，是錯位**——每個字都在，只是掛錯選項。所以**多重集守恆那類閘門
完全看不見它**，而讀起來又「幾乎像對的」。規模實測：ast 59 題數學裡 26 題中招（44%）、
cap 32 題、gsat 15 題；20 份數學卷共 998 條分數線。

### 判準：分數線是畫上去的物件，但**分類不要用幾何容差**

沿用本檔的「原生物件層優先」——分數線就是一條細長水平向量。找到它靠物件層，
但**它是哪一種，要問語義上本來就該問的那句話**：

| 類別 | 判準 |
| --- | --- |
| 分數 `\dfrac{}{}` | 線的上方**有分子** |
| 根號 `\sqrt{}` | 上方沒有東西，左緣接一段向下的筆畫 |
| 線段記號 `\overline{}` | 上方沒有東西，左緣也沒有筆畫 |
| 表格線／底線 | 沒有被它框住的字 |

> 🔴 **這一節原本寫的是一組幾何門檻，而那是解錯問題。**（2026-08-18 被反駁式審查推翻，
> 保留在這裡是因為推翻的過程比結論值錢。）
> 第一版拿「上槓緊貼、分數線隔一個字高」當判準，在校準的那一題上兩群是
> 0.35–0.47 對 1.54–1.58——差三倍、不重疊、看起來乾淨得不得了。
> 攤開**同一份卷**的其他 21 條就散掉：真分數的 gap 低到 0.31（分母首字是「−」「(」
> 這種等高框字形時，bbox 頂端幾乎頂到線），上槓最大到 0.48，**完全重疊**。
> 被誤判的其中一題，正是那一輪要修的目標題之一——**工具原本會把它弄得更糟。**
>
> **一般形式：校準樣本只有一題，跟沒校準的差別沒有想像中大。**
> 門檻在單一樣本上「乾淨地分成兩群」不是證據，那是樣本數不足的正常長相。
> 訂門檻之前先把同一份資料的**全部**案例攤開看分布；分不開就換判準，不要調門檻。

配套的過濾是語義的，不是容差：**分子不會超出它自己的分數線**（上一行的散文會，
因為它是一條長文字，只是剛好有一兩個字落進 bar 的 x 範圍）。

> 🔴 **另一條「分子不會是漢字」也被推翻了**（2026-08-18 同一輪）。反例：
> `114_會考數學` p11 題1 的原卷就是**中文寫的分數**（「所有受訪者數／全體成年人口
> 總數」）。扣掉漢字分子會讓它掉成 `\overline{全體成年人口總數}`，而那長得完全像對的。
> 中文分數存在，散文溢入也存在，這裡分不開——**分不開的時候降人工，
> 不要挑一個比較常見的猜。**

**取字窗跟字級走，不要跟行距走。**（原本寫「用本頁正文行距逐頁量」，2026-08-18 推翻。）
行距**要多行才量得到**，於是在短帶上崩壞，而且兩個方向都會寫壞題目：太小 →
真分子落在窗外，`\dfrac{x-y^{2}}{10}` 掉成 `\overline{10}`；太大 → 上一行的作答
圈號變成分子。實跑 19 卷，行距版的窗實測散佈 6.5–30.8，正常帶才 11–16。
分子的物理量本來就不是行距，是**分數自己的高度**，那跟字級走（實測正文 10–12pt
時分子距 7.5–8.7pt、會考 14pt 時 13.4–14.1pt），而字級**一行就量得到**，再上下夾死。

> 一般形式：**一個量能不能當判準，要先問它在最小的樣本上量不量得出來。**
> 需要多筆資料才算得出的統計量（行距、中位數、分布），在稀疏的個案上不是不準，
> 是**會給出一個看起來正常的錯值**——而錯值不會報錯。

**根號與線段記號一定要分開。** 「令BC=a」在原卷是 \overline{BC}，當成根號會寫出
`\sqrt{BC}`——**比不修還糟**。自檢要對同一條上槓正反各驗一次（有筆畫→根號、
拿掉筆畫→線段記號），否則不知道這一格到底有沒有在工作。

### 🔴 √ 不是字元，是向量筆畫

實測整頁的 SymbolMT PUA 裡**沒有任何 √**——它由三段 stroke 畫成再接上槓。
**找字元永遠找不到它。** 這條的代價是實際發生的：入庫成「3」而不是「√3」，
紅隊看到詳解寫 √3 卻在題幹找不到，開了一則 input_gap，差點害我們去改一則**正確**的詳解。

> 一般形式：**符號不一定是字。** 決定「這個東西是什麼」之前，先確認它在 PDF 裡
> 到底是 glyph 還是 path——兩者要用完全不同的 API 去找，而找錯的那一邊永遠回空。

### 同一份卷可以有不只一種堆疊——解掉一種不等於這一題好了

實測（2026-08-18，ast 分科數學）：分數與根號都重建對了、整題級等式閘也過了，
逐題比對原卷仍然**只有 1/9 是乾淨的**。另外 8 題錯在兩件與分數無關的事：

- **上標與絕對值也是跨行結構**：`|z| = 2` 在物件層是 `2 ⏎ = ⏎ z` **三行**。
  分數有 bar 當錨點，這一種沒有，歸行時整個被指到別行去，於是輸出
  `設複數z的虛部不為0且z2`——`=` 與兩條豎線都不見了。
- **帶界本身就切錯**：以題號錨點的 y 切帶＝預設單欄閱讀順序，實測多題的選項 (5)
  尾巴掛著**下一題**的內容。

> 一般形式：**修好一種結構之後，剩下的錯不會自己標示自己來自哪一層。**
> 它們全部長成「這個工具做壞了」。收工前一定要逐題比對原卷，
> 否則會拿一個只在校準題上成立的結論去推廣。
> 而且**只有一題乾淨的那一題，正是校準用的那一題**——這就是校準樣本的長相。

### 帶界不能用「某個字的 y」來切

以題號那個字的頂端當題帶邊界，等於假設**同一行的字頂一樣高**。數學卷不是：
**下一題的上標比它自己的題號還高**（19 卷實測，同列抬升 6–10pt 的字有 619 個，
5pt 處只有 3 個）。於是下一題的指數落進上一題的帶，上一題的最後一個選項尾巴
就掛著下一題的式子——不報錯，讀起來像那題本來就很長。

正解不是調一個容差，而是**先歸行再切**：以正文字級叢集出行，題號屬於哪一行，
那一行**最高的字頂**就是邊界。「一個字屬於哪一行」是算得出來的，不必猜。
同一個修正順帶解掉反向的病（自己第一行的分子在自己的題號之上而被切掉）。

> 一般形式：**凡是拿「某個代表性元素的座標」當群體邊界的地方，都藏著
> 「這個群體是齊平的」這個假設**，而排版裡的上標、下標、分數、根號全都不齊平。

### 具名排除桶：等式閘的另一半

整題級的閘要寫成**等式**（新內容 ＋ 排除桶 ＝ 原帶），而等式成立的前提是
**每個被丟掉的字元都有名有姓**。實測丟棄點共五處，最大的一桶最容易漏：
頁碼／圖說整行、**章節界線之後未讀的所有行**、圖框內的標籤、選項標記、題號。

跑起來當天就抓到 `113_分科數學甲_8` 掉了 51 個字元——而包含關係那道閘
（新 ⊆ 原帶）**一聲都沒吭**，因為漏抄正是它看不見的方向。

⚠️ 但排除桶也有它自己的洞：**丟錯了卻有名字的字元，等式一樣成立**。
實測撞到：`112_分科數學甲_4` 的 `-10` 掉了 `1` 與 `0`，兩個字元有名有姓地躺在
「圖內標籤」桶裡，等式照樣過。真兇是**根號的填充路徑本身就是一個圖框**，
框裡的字被「這個字畫在圖上」那條規則吃掉——那條規則是為了擋圖上的標籤寫的。
補法不是去分辨哪些該丟，是**在這個情境下那個桶本來就該是空的**：
數學帶不該有圖內標籤，非空就代表判準用錯地方，直接拒收。

所以等式閘是必要條件，不是驗收條件；驗收仍然是逐題比對原卷。

### 這一組閘門值得記的四個實測案例（都是「合法輸出、意思不同」）

| 閘 | 抓到的東西 | 為什麼守恆閘看不見 |
| --- | --- | --- |
| 圖內標籤桶須為空 | `-10` 掉了兩個字元 | 丟棄有名字，等式成立 |
| 懸空的向量 accent | 尾巴出現 `\vec \vec \vec` | accent 與它的字母被拆到不同行；純文字投影裡是三個 `→` |
| 分數後緊接上下標 | `x²/9` 變成 `\dfrac{x}{9}^{2}` ＝ `(x/9)²` | 字元一個不少，**是另一個方程式**，而且渲染得漂漂亮亮 |
| 帶內沒解讀的豎線 | `|x-a|≤b` 變成 `x-a≤b` | 絕對值的豎線是**畫出來的**，根本不在字元流裡 |

> 🔴 最後一列是這一整節的縮影：**守恆閘只能守「在字元流裡的東西」。**
> 凡是用畫的（分數線、上槓、絕對值、括號延伸件、根號的勾），它一律看不見。
> 所以每一種「畫出來的數學符號」都要嘛被解讀、要嘛被具名拒收——沒有第三條路。

驗收方法本身也值得記：**把目視判斷與閘門判斷拿來對帳。** 實測 13 題裡
9 題目視判錯／全被拒收、4 題目視判乾淨／全過閘——完全一致。
「閘全過」不算驗收過，「閘與人的判斷在同一批資料上一致」才算。

### 排除規則只認得一種寫法，等於它在別的卷上不存在

同一套排除規則跨到別的考試家族就失效，而失效的形狀是**把版面家具當成內容**：

| 家具 | 原規則只認 | 別的卷長什麼樣 | 後果 |
| --- | --- | --- | --- |
| 圖說 | `圖16` | 會考的 `圖( 六)`（括號、國字、括號後還有空白）| 圖說被接到最後一個選項後面 |
| 頁碼 | `- 12 -` | 會考是光禿禿一個數字 | 頁碼變成最後一個選項的尾巴（`9√15` → `9√151`）|
| 大題標題 | `第壹部分`／`說明：` | `三、選填題（占18 分）` | 整段被黏進上一題的選項 (5) |

三個都是同一種失效：**規則是照著第一份卷寫的，而它不會在第二份卷上報錯。**
補法有兩種，選哪一種要看那支規則還服務誰：能安全放寬就放寬（圖說），
會影響別的線就在自己這條路上**具名拒收**（頁碼——同一個樣式在自然科可能是真內容）。

### 對帳型判準要先問「有沒有東西可對」

選項標記有兩種寫法（`(A)`–`(E)` 與 `(1)`–`(5)`），挑哪一種**不要猜**——
拿「切出來的選項數 == 題庫記的選項數」對帳就好，這是個很乾淨的判準。
但它有個沒寫出來的前提：**題有選項**。選填題與非選題的選項數是 0，兩種樣式都
切出 0 個，於是判準回報「兩種都對，分不出來」，把它們全數擋掉——
實測 112 題（佔該科數學題四分之一以上）**永遠進不了門，而且理由看起來很合理**。

> 一般形式：對帳、投票、比較這類判準，在**退化輸入**（空集合、單一候選、全相同）
> 上會給出一個看起來像「失敗」的答案，而它其實是「不適用」。
> 兩者在程式裡長得一樣，在報表上也長得一樣——差別只有沒人去問「這裡有東西可對嗎」。

### 位置錯了，字元一個沒少：選填題的作答格

`○15-1`、`○15-2` 是學生**填答案的位置**。它們印在右側或下一行，歸行時很容易被塞進
句子中間——實測題幹變成「…、○15-1(c,log6c)在…之值為。（化為最簡分數）○15-2」，
兩格分家、而題目本身變成「之值為。」；另一題把「隨機」切成兩半。
守恆閘全過，因為一個字都沒少。

閘門寫法：**所有作答格必須連成一段**。真的分佈在兩處的題會被誤拒，那是可接受的代價
——填答格的位置錯了，題目就不能作答，比少修一題嚴重。

### 剝題號要查排除桶，不要查字串

題號是靠 `^{題號}[.．、]` 比對重建後的 token 串開頭剝掉的，而題號**可能被合成
token 吞進式子裡**——實測 `會考_111_數學_6` 的根號上槓罩住了題號那一列，
題幹變成 `\(6\sqrt{2022}.\)的值介於…`，那個 6 是題號不是係數。
比對失敗不會報錯，只會安靜地不剝。
閘門要問的不是「開頭還有沒有題號」，而是**「剝題號這件事到底發生了沒有」**——
查排除桶裡有沒有那一筆，比在輸出字串上找痕跡可靠。

### 接寫回之前要先改守恆閘

上下標重建的 W10 閘驗「重建後的字元多重集 ⊆ 題帶的字元多重集」，用來抓漏抄。
數學重建**刻意**增加字元（`\dfrac{}{}`、`\sqrt{}`、`^{}`），照原判準必被擋下。
正確改法是**把合成的 LaTeX 語法從比對扣掉、只驗內容字元守恆**——
不是把閘關掉，它守的「有沒有字不見」在數學題一樣會發生。

### 這類重建器的預設立場必須是「拒收」

它改的是**學生看得到的題目本文**，而失敗長相是「讀起來幾乎像對的」——
`\sqrt{BC}` 與 `\overline{BC}` 只差一個字，分子掛到隔壁選項則每個字都還在。
🔴 **任何字元守恆閘都看不見這種錯**（多重集完全相同），所以唯一的防線是
**不確定就不動、降 `review` 讓人看**。四種必須拒收的形狀，都是實測踩到的：

- **表格**：3 條以上水平線**起點與寬度都**相同，或有豎線與之相交。
  只看起點會誤殺選項列——同一題的分數線本來就左邊對齊。
- **巢狀**：bar 的取字範圍內還有另一條 bar。先處理的那條會把字搶走，
  後處理的那條空手，於是分母變孤兒字元——**修復器自己重演了原病灶**。
- **分數線找不到分子**：不可靜默 `continue`。
- **版面家具**：段落分隔線寬達 471pt，具名略過。

厚度**上限**要按考試家族量：會考有厚到 3.09pt 的線，用分科／學測的 2.0pt 上限
會讓整批隱形——而隱形是靜默丟失，輸出 `ACADAB` 沒有任何閘會叫。

> 🔴 **但不要把那份厚度分布當成分類器。** 同一輪的審查者據此提了一條備援判準
> 「會考家族內靠厚度雙峰分工：h≈0 是分數線、h=3.09 是線段記號」（實測分布
> {≈0: 372, 3.09: 47} 確實是雙峰）。**實測否證**：`111_會考數學` p5 題13 的
> `AB`、`AC` 線段記號量到 **h=0.00**，與分數線同厚。分布是真的，3.09 那一群
> 不是線段記號。**量得出分布 ≠ 分得開兩類**——雙峰只說明有兩種東西，
> 沒說明是哪兩種，而中間那一步是要另外驗的。

### 上槓 vs 向量記號：**向量根本不是畫的，是一個字元**

> 🔴 本節在同一天內被自己推翻兩次，兩次的過程都比結論值錢，所以都留著。
>
> **第一版**（被推翻）：`\overline{OB}` 與 `\vec{OB}` 在物件層長得一模一樣——
> `get_drawings()`、`extended=True`、`get_texttrace()`、rawdict 四層都只有 shaft
> 一條線，找不到箭頭。結論是「只能靠課綱先驗：會考沒有向量課綱，分科／學測有」，
> 於是分科／學測的上槓一律不動。
>
> **推翻它的是一個計數**：向量記號是 **MT-Extra 字型的 0xF086**，一個加在下一個
> 字母上的箭頭 accent——**它是字元，不是線**。全 19 卷數學實測：
> ast 40 個／gsat 17 個／cap **0** 個（會考沒有向量課綱，數字自己對上了）。
> 而當初拿來證明「箭頭不在物件層」的那個個案（`112_數學A` p6 的 `OB`），
> 同一列有五個 accent 落在**別的** OA／OB／OC 上、唯獨它沒有——
> **它本來就是線段記號**。找不到箭頭，是因為那裡沒有箭頭。

判準因此變成一句話：**罩住的字帶著 0xF086 accent ⇒ 向量；否則就是線段記號。**
課綱先驗整條刪掉，可自動處理的帶從 83 升到 93。

> 一般形式：本檔另一節寫過「**符號不一定是字**」（√ 是三段筆畫，找字元永遠找不到）。
> 這一則是它的反面——**線也不一定是畫的**。在斷定「這個東西找不到」之前，
> 先確認你找的型別對不對；型別錯的那一邊**永遠回空**，而空值長得很像結論。
>
> 更貴的一課：找不到證據時，最順手的替代品是**先驗**（課綱、常識、慣例）。
> 先驗會給出一個看起來很有道理、而且**永遠不會被資料反駁**的規則——
> 因為它根本沒在看資料。上面那條先驗如果沒被這個計數撞到，會一直正確地
> 拒收 236 條 gsat 上槓，而且沒有任何一道閘會叫。

### 拒收也有代價，所以判準要是「有證據」不是「有風險」

同一輪加的一道拒收「取字窗越過帶上緣 ⇒ 分子可能被切掉」實測**誤殺 214 條**——
因為題目**第一行**本來就是分數最常出現的地方，而帶界就切在第一行的頂端。
改成「帶上緣之外**真的有字**落在這條 bar 的取字範圍內」之後降到 119 條，
而它要抓的那個真案例（`114_分科數學乙` p5 題10 的 `(1−7i)/(−1+i)` 被切成
`\overline{−1+i}`）仍然抓得到。

> 一般形式：**「不確定就拒收」不等於「有風險就拒收」。** 前者要求你先取得
> 「這裡確實不確定」的證據；後者只要求你想像得出一個壞情況——而壞情況永遠想像得出來。
> 拒收率衝高不會有人抱怨（沒有閘會叫），但覆蓋率歸零跟工具不存在是一樣的。

**拒收的粒度也要分兩種**：**整帶拒收**（這一帶的判讀不可信：表格、巢狀、分子被切掉）
與**略過單一結構**（這一處沒把握、它的字原封不動留著，其餘照修）。
向量歧義屬於後者——若整帶拒收，校準案例 `114_分科數學乙_8`（題幹有線段記號、
選項有四個分數）會一條都修不到。判準：**不動它就等於現況**的，略過即可；
**會讓同帶其他結構取錯字**的，整帶拒收。

### 有沒解讀的墨水疊在剛重建的結構上 ⇒ 你沒看懂它

實測 `111_分科數學甲` p3 題3：五個選項是 √2/4、√3/3、√6/6、√6/3，而**那份卷的
根號是單一個高物件**（不像校準卷拆成三段筆畫），高度超過 bar 上限就不在視野裡。
於是合成出 `\dfrac{3}{3}` 與 `\dfrac{6}{6}`——**同一題的兩個選項都等於 1**。

閘門寫法：重建完一個結構後，檢查它消耗掉的字元 bbox 上有沒有**不是細長水平線**的
繪圖物件。細長水平線是我們認得的（分數線、上槓），其餘的墨水代表這裡還有一層沒解讀。

> 一般形式：同一個符號在**不同年份的同一種卷**可以有完全不同的物件結構。
> 「這份卷的根號是三段筆畫」是那份卷的事實，不是那個符號的事實。

### 接寫回之前要先改守恆閘

上下標重建的 W10 閘驗「重建後的字元多重集 ⊆ 題帶的字元多重集」。數學重建**刻意**
增加字元（`\dfrac{}{}`、`\sqrt{}`、`^{}`），照原判準必被擋下。

**改法不是事後用 regex 把語法剝掉**——題帶本來就含 `{}`（集合）與 `^`，剝完會
分不出「我合成的」與「原卷就有的」，一剝就把後者的合法性證據一起剝掉；
反過來，無中生有的字元只要長得像語法就被赦免。

**正確做法：合成 token 時同步存一份 `plain` 投影**（PUA 走純文字欄），
純文字輸出用它而不是用 LaTeX；閘改成**等式**而非包含關係：
`multiset(重建 plain) + multiset(具名排除桶) == multiset(題帶字元)`。
排除桶＝頁碼、圖說、圖內標籤等，每個被丟棄的字元有名有姓。
這樣同時擋漏抄、**重複消耗**（同一個字被兩條 bar 各取一次，⊆ 方向永遠過）與圖框誤吞。

## 八、紅色浮水印濾除：色彩判準要用通道飽和度，不是色相或「非中性色」

**判準：R 通道是否飽和**（`red_mask`：R>=250 且 R 顯著高於 G／B），不是「紅不紅」、更不
是「非中性色」。**抹除要鄰域反覆平均補回底色，不可塗白**（灰底要補灰）。**同色但屬真內
容的科別要先全科掃過再套用，不可盲套同一濾網。**

判準踩過兩次坑（2e999e6）：**「非中性色」不行**——`112_*_國語文_05` 有 16 萬個非中性
像素卻沒有浮水印，它本來就是彩圖；**「紅」也不行**——49 張教檢圖裡 14 張帶紅，3 張的
紅是**題目本身**（紅色折線資料、西瓜果肉、秤面紅刻度指針），濾掉會洗掉答案。真正分界
是 R 通道飽和：浮水印一律 R=255（p10 也有 251），內容紅的 R 中位數是 192／70／240。

**塗白也不行**：115 q12 的圓餅圖是灰階填色，塗白會切出白色刀痕，「黑墨沒少」那條斷言
看不見；改鄰域反覆平均補色，`assert (b[~m]==a[~m]).all()` 守住 mask 外一個像素都不准動。

**「先掃過再套用」的依據**（717b2b6）：doctor 科全科掃描 415 張圖，51 張命中飽和紅，
逐張看全是心電圖紅色格線紙與病理／臨床照，都是真內容——script 明寫「不要對 doctor
跑」，判準對但沒先掃過全科照樣誤傷。

selftest（`strip_red_watermark.py --selftest`，4 條）逐條否證：浮水印紅抓到並補回底色、
內容紅一個像素都不准動、灰底補灰不補白、非紅彩圖不動。

🔴 **修正**：同一 commit 的 `crop_teacher_scan_figs.py` 之 `dewatermark()` 其實用較簡單
的「非中性色」門檻，因為只套在 7 個已知單一圖表的窄裁切窗，不是盲套全科；R 通道飽和
判準是廣用／盲掃時才必需的加強，兩者不矛盾。

## 九、掃描卷沒有物件層可查，改用墨跡列高度剖面找圖的上下界

**掃描卷裁圖的上下界不能信任文字層 bbox。** 這是三、「band 邊界怎麼決定」裡「圖底保護」
的同一條規則在掃描卷上的延伸——三用 `get_images()`／`get_drawings()` 查真實圖／向量物件
的 bbox 去頂開文字給的硬上限；但掃描卷整頁只是一張點陣圖，沒有物件層可查，得換一個訊號。

**改用墨跡列高度剖面**（`crop_teacher_scan_figs.py`）：先塗掉浮水印（見八），把墨跡切成
被空白（`GAP=5px`）隔開的列群，**第一個高度 > `MIN_H`（40px @200dpi）的群就是圖**——正
文永遠是單行高，量不到 40px；`gap<=ABSORB`（16px）的矮群（座標軸刻度、圖說）收進同一張
圖，gap 23–35px 的下一段正文擋在窗外。

實測依據（2e999e6）：112 q12「已知某班」那個 block 的文字 bbox `y0=209`，圖其實延到
230 以下——block bbox 蓋到圖上，用它當下界會切圖。111 q11 則是靠 ABSORB 這條分寸，才
沒把 0–100 的刻度軸誤判成下一段正文而切掉。

**窗會動態放大**：若群貼著搜尋窗（錨點行底 → 下一題題號頂）的邊緣，代表圖可能還沒收
全，窗上下各退 24px 重找，最多 4 輪，直到高度不再變化。收工前用
`assert not (r0 < 3 or r1 > len(ink)-4)` 擋住「圖仍貼著窗緣」——硬失敗好過默默出錯圖。

## 教訓索引（已搬至 figures-lessons.md）

- figures-lessons.md §十：碰到物件不代表整個物件都屬於本題（2026-09-10）
- figures-lessons.md §十一：裁切完整性要量在缺陷尚未被補白掩蓋的影像上（2026-09-10）
- figures-lessons.md §十二：點陣圖裡的軸標籤不會出現在 PDF 文字層（2026-09-09）
