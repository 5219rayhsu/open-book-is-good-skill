...
### 上槓判不出「線段記號」還是「向量記號」——而箭頭不在物件層

`\overline{OB}` 與 `\vec{OB}` 在物件層長得一模一樣：`get_drawings()`、
`extended=True`、`get_texttrace()`、rawdict **四層都只有 shaft 一條線**，
找不到箭頭（頭部在 Type3／XObject 之外或被 parser 丟棄）。所以「右緣有沒有箭頭」
這條判準**做不出來**。可用的只有**課綱先驗**：會考沒有向量課綱，分科／學測有。
