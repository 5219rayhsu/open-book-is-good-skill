...
> 本檔 347 行。`head -100` 看不到全貌，故列出所有節名——**用節名搜尋，不要從頭讀**。
...
引用同組兄弟題者（`承上題`／`依前題`／`根據第 N 小題`／`同上題條件`／`利用前小題的結果`）
標 `false` → 平臺在**模擬題／單題／錯題／弱點**等模式必須**整組一起出、不拆**；
其餘標 `true`。

🔴 **關鍵分辨**：「上述／如圖」指的是**共用材料**（passage／圖 → 仍可單獨），
還是**前一子題的內容或答案**（→ 不可單獨）。這是判斷式，在解析時一次標好。
